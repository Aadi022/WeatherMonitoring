  ...
  useEffect(() => {
    const fetchWeatherData = async () => {
      try {
        ...
        const cityWeather = response.data.find(w => w.city === 'Bangalore');
        ...
        if (cityWeather.maxTemp > cityWeather.maxThreshold) {
          alert(`ALERT: The temperature in Bangalore is above the max threshold of ${cityWeather.maxThreshold}°C! Current temp: ${cityWeather.maxTemp.toFixed(2)}°C`);
        }
        if (cityWeather.minTemp < cityWeather.minThreshold) {
          alert(`ALERT: The temperature in Bangalore is below the min threshold of ${cityWeather.minThreshold}°C! Current temp: ${cityWeather.minTemp.toFixed(2)}°C`);
        }
      } catch (error) {
