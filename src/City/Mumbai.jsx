  ...
  useEffect(() => {
    const fetchWeatherData = async () => {
      try {
        ...
        const cityWeather = response.data.find(w => w.city === 'Mumbai');
        ...
        if (cityWeather.maxTemp > cityWeather.maxThreshold) {
          alert(`ALERT: The temperature in Mumbai is above the max threshold of ${cityWeather.maxThreshold}°C! Current temp: ${cityWeather.maxTemp.toFixed(2)}°C`);
        }
        if (cityWeather.minTemp < cityWeather.minThreshold) {
          alert(`ALERT: The temperature in Mumbai is below the min threshold of ${cityWeather.minThreshold}°C! Current temp: ${cityWeather.minTemp.toFixed(2)}°C`);
        }
      } catch (error) {
